const d={name:"LeadAssignmentSettings",props:{salesUsers:{type:Array,required:!0},leadAssignmentConfig:{type:Object,required:!0}},data(){return{form:{enabled:this.leadAssignmentConfig["lead_assignment.enabled"]==1,method:this.leadAssignmentConfig["lead_assignment.method"]||"round_robin"},search:"",selectedUserIds:this.leadAssignmentConfig["lead_assignment.active_users"]?JSON.parse(this.leadAssignmentConfig["lead_assignment.active_users"]):[],userStars:this.initUserStars()}},computed:{filteredUsers(){if(!this.search)return this.salesUsers;const e=this.search.toLowerCase();return this.salesUsers.filter(t=>(t.name+" "+t.email).toLowerCase().includes(e))},isAllSelected(){return this.filteredUsers.length>0&&this.filteredUsers.every(e=>this.selectedUserIds.includes(e.id))},roundRobinPercent(){const e=this.selectedUserIds.length||this.filteredUsers.length;return e>0?Math.floor(100/e):0}},mounted(){console.log("[LeadAssignmentSettings] Component mounted successfully!"),console.log("[LeadAssignmentSettings] Props received:",{salesUsers:this.salesUsers,config:this.leadAssignmentConfig})},methods:{initUserStars(){console.log("[LeadAssignmentSettings] Initializing user stars...");let e={};try{e=this.leadAssignmentConfig["lead_assignment.weights"]?JSON.parse(this.leadAssignmentConfig["lead_assignment.weights"]):{}}catch(s){console.error("[LeadAssignmentSettings] Error parsing weights:",s),e={}}const t={};for(const s of this.salesUsers)t[s.id]=e[s.id]?Math.max(1,Math.min(5,Math.round(e[s.id]/20))):1;return t},toggleSelectAll(){this.isAllSelected?this.selectedUserIds=[]:this.selectedUserIds=this.filteredUsers.map(e=>e.id)},clearAll(){this.selectedUserIds=[]},setUserStars(e,t){this.$set(this.userStars,e,t)},weightedPercent(e){const t=this.selectedUserIds.reduce((r,a)=>r+(this.userStars[a]||1),0);if(!this.selectedUserIds.includes(e)||t===0)return 0;const s=this.userStars[e]||1;return Math.round(s/t*100)},submitForm(){this.$emit("submit",{...this.form,active_users:this.selectedUserIds,weights:Object.fromEntries(this.selectedUserIds.map(e=>[e,(this.userStars[e]||1)*20]))})}},template:`
        <div>
    ...
    `};window.app?(console.log("[LeadAssignment] Registering component to window.app"),window.app.component("lead-assignment-settings",d)):console.warn("[LeadAssignment] window.app not found, component registration delayed");
