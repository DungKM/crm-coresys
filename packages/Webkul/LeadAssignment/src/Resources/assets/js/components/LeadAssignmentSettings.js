export default {
    ...
    data() {
        ...
        try {
            ...
            if (activeUsersValue) {
                activeUsers =
                    typeof activeUsersValue === "string"
                        ? JSON.parse(activeUsersValue)
                        : activeUsersValue;
            }
        } catch (e) {
            console.error(
                "[LeadAssignmentSettings] Error parsing active_users:",
                e
            );
        }
        ...
        return {
            ...
            search: "",
            selectedUserIds: Array.isArray(activeUsers)
                ? activeUsers.map((id) => parseInt(id))
                : [],
            userStars: {},
            hoveredStar: {},
        };
        ...
        initUserStars() {
            ...
            for (const user of this.salesUsers) {
                stars[user.id] = weights[user.id]
                    ? Math.max(1, Math.min(5, parseInt(weights[user.id])))
                    : 1;
            }
            return stars;
        },
