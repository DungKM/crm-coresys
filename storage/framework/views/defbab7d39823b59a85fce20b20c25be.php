...
<?php $component->withAttributes([]); ?>
     <?php $__env->slot('title', null, []); ?> 
        <?php echo app('translator')->get('admin::app.leads.chat.title'); ?>
     <?php $__env->endSlot(); ?>
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'admin::components.breadcrumbs.index','data' => ['name' => 'admin.leads.chat.index','entity' => $lead]] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? (array) $attributes->getIterator() : [])); ?>
...
<?php $attributes = $attributes->except(collect($constructor->getParameters())->map->getName()->all()); ?>
<?php endif; ?>
<?php $component->withAttributes(['name' => 'admin.leads.chat.index','entity' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($lead)]); ?>
...
<?php endif; ?>
            ...
            <div class="w-full rounded-md border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900">
                ...
                <div class="bg-white dark:bg-gray-900 rounded-b-lg flex flex-col" style="height: calc(100vh - 180px);">
                    ...
                    <div id="pinned-banner-container">
                        <?php if($pinnedMessages->isNotEmpty()): ?>
                            <div class="pinned-messages-banner" id="pinned-banner">
                                <?php $__currentLoopData = $pinnedMessages; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $pinned): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <?php
                                    ...
                                    ?>
                                    <div class="pinned-item" onclick="scrollToMessage(<?php echo e($pinned->id); ?>)" data-pinned-id="<?php echo e($pinned->id); ?>">
                                        <span class="text-amber-600 dark:text-amber-400">📌</span>
                                        <span class="pinned-item-content">
                                            <strong><?php echo e($pinnedIsIncoming ? 'Khách hàng' : ($pinned->user->name ?? 'Bạn')); ?>:</strong>
                                            <?php echo e(Str::limit($pinnedText, 60)); ?>

                                        </span>
                                        <button type="button" class="pinned-item-unpin" onclick="event.stopPropagation(); togglePin(<?php echo e($pinned->id); ?>)" title="Bỏ ghim">
                        ...
                        <?php else: ?>
                            <?php $__currentLoopData = $whatsappLogs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $log): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                ...
                                <div class="flex flex-col <?php echo e($isIncoming ? 'items-start' : 'items-end'); ?>"
                                    data-message-id="<?php echo e($log->id); ?>">
                                    ...
                                    <div
                                        class="relative flex items-start gap-1 <?php echo e($isIncoming ? '' : 'flex-row-reverse'); ?>">
                                        
                                        <div
                                            class="max-w-[75%] rounded-lg px-4 py-2 text-sm shadow-sm <?php echo e($isIncoming ? 'chat-message-incoming bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-tl-none' : 'chat-message-outgoing bg-blue-600 dark:bg-blue-500 rounded-tr-none'); ?>">
                                            <?php if($mediaHtml): ?>
                                                <?php echo $mediaHtml; ?>

                                            <?php endif; ?>
                                            <?php if($textContent): ?>
                                                <span class="whitespace-pre-wrap"><?php echo e($textContent); ?></span>
                                            <?php endif; ?>
                                        </div>

                                        
                                        <div class="flex-shrink-0 relative">
                                            <button type="button" onclick="toggleMessageMenu(<?php echo e($log->id); ?>)"
                                            ...
                                            <div id="menu-<?php echo e($log->id); ?>"
                                                class="hidden absolute <?php echo e($isIncoming ? 'left-0' : 'right-0'); ?> top-full mt-1 w-40 rounded-lg shadow-2xl border border-gray-200 dark:border-gray-700"
                                                style="z-index: 9999; background: #ffffff; isolation: isolate;">
                                                <div class="rounded-lg overflow-hidden" style="background: #ffffff;">
                                                    <style>
                                                        .dark #menu-<?php echo e($log->id); ?>,
                                                        .dark #menu-<?php echo e($log->id); ?> > div { background: #1f2937 !important; }
                                                    </style>
                                                <button onclick="copyMessage(<?php echo e($log->id); ?>)"
                                                    class="w-full px-3 py-2 text-left text-sm hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-2 rounded-t-lg">
                                                    <span>📋</span> Sao chép
                                                </button>
                                                <button onclick="togglePin(<?php echo e($log->id); ?>)"
                                                    class="w-full px-3 py-2 text-left text-sm hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-2">
                                                    <span>📌</span> <?php echo e($isPinned ? 'Bỏ ghim' : 'Ghim'); ?>

                                                </button>
                                                <button onclick="toggleStar(<?php echo e($log->id); ?>)"
                                                    class="w-full px-3 py-2 text-left text-sm hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-2">
                                                    <span>⭐</span> <?php echo e($isStarred ? 'Bỏ sao' : 'Gắn sao'); ?>

                                                </button>
                                                <button onclick="showMessageInfo(<?php echo e($log->id); ?>)"
                                                    class="w-full px-3 py-2 text-left text-sm hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-2">
                                                    <span>ℹ️</span> Thông tin
                                                </button>
                                                <button onclick="showForwardModal(<?php echo e($log->id); ?>)"
                                                    class="w-full px-3 py-2 text-left text-sm hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-2">
                                                    <span>↪️</span> Chuyển tiếp
                                                </button>
                                                <button onclick="deleteMessage(<?php echo e($log->id); ?>)"
    ...
    <?php if (! $__env->hasRenderedOnce('9dbcc7ef-8a17-4f87-b615-727c8e6b2481')): $__env->markAsRenderedOnce('9dbcc7ef-8a17-4f87-b615-727c8e6b2481');
$__env->startPush('scripts'); ?>
        <script>
            // Biến toàn cục để theo dõi tin nhắn cuối cùng
            window.lastMessageId = <?php echo e($lead->activities->where('type', 'whatsapp')->max('id') ?? 0); ?>;
            window.leadId = <?php echo e($lead->id); ?>;
...
<?php endif; ?><?php /**PATH C:\laragon\www\base_crm1.0\packages\Webkul\Admin\src/resources/views/leads/chat.blade.php ENDPATH**/ ?>