...
class WhatsAppController extends Controller
{
    ...
    public function handleIncomingMessage(Request $request)
    {
        ...
        if ($person) {
            ...
            // Tạo Activity lưu tin nhắn
            $activity = $this->activityRepository->create([
                'title'          => 'Tin nhắn WhatsApp đến', // Tiêu đề này dùng để phân biệt tin đến/đi ở giao diện
                'type'           => 'whatsapp',
                'comment'        => $messageContent,
                'user_id'        => $lead ? $lead->user_id : 1, // Gán cho sale phụ trách
                'is_done'        => 1,
            ]);

            Log::info("[WhatsApp Webhook] ✅ Created Activity ID: {$activity->id}");
    ...
    // ==========================================
    // 6. REPLY TIN NHẮN TỪ AJAX (Tab WhatsApp Chat)
    // ==========================================
