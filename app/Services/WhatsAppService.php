...
class WhatsAppService
{
     ...
     * @param string|null $replyToMessageId - WhatsApp Message ID của tin nhắn cần reply (optional)
     * Return: ['success' => true/false, 'message' => 'message', 'error_code' => code, 'whatsapp_message_id' => wamid]
     */
    public function sendTextMessage($toPhone, $message = 'Xin chào!', $replyToMessageId = null)
    {
        $url = "{$this->apiUrl}/{$this->phoneId}/messages";

        Log::info("[WhatsApp] Sending TEXT message to phone: {$toPhone}" . ($replyToMessageId ? " (Reply to: {$replyToMessageId})" : ""));
        ...
        for ($attempt = 1; $attempt <= $maxRetries; $attempt++) {
            ...
            try {
                ...
                // Thêm context nếu đây là tin nhắn reply
                if ($replyToMessageId) {
                    $payload['context'] = [
                        'message_id' => $replyToMessageId
                    ];
                    Log::info("[WhatsApp] Including reply context: {$replyToMessageId}");
                }
                
                Log::info("[WhatsApp] Full payload: " . json_encode($payload));
                ...
                if ($jsonData && isset($jsonData['messages']) && is_array($jsonData['messages']) && count($jsonData['messages']) > 0) {
                    $wamid = $jsonData['messages'][0]['id'] ?? null;
                    Log::info("[WhatsApp] ✅ SUCCESS - Message ID: " . ($wamid ?? 'unknown'));
                    return ['success' => true, 'message' => 'Tin nhắn đã gửi thành công', 'whatsapp_message_id' => $wamid];
                }
    ...
    public function sendDocumentMessage($toPhone, $mediaId, $filename, $caption = null)
    {
        ...
        try {
            ...
            if ($jsonData && isset($jsonData['messages']) && count($jsonData['messages']) > 0) {
                $wamid = $jsonData['messages'][0]['id'] ?? null;
                Log::info("[WhatsApp] ✅ Document sent - Message ID: " . ($wamid ?? 'unknown'));
                return ['success' => true, 'message' => 'Document đã gửi thành công', 'whatsapp_message_id' => $wamid];
            }
    ...
    public function sendImageMessage($toPhone, $mediaId, $caption = null)
    {
        ...
        try {
            ...
            if ($jsonData && isset($jsonData['messages']) && count($jsonData['messages']) > 0) {
                $wamid = $jsonData['messages'][0]['id'] ?? null;
                Log::info("[WhatsApp] ✅ Image sent - Message ID: " . ($wamid ?? 'unknown'));
                return ['success' => true, 'message' => 'Hình ảnh đã gửi thành công', 'whatsapp_message_id' => $wamid];
            }
    ...
    public function sendVideoMessage($toPhone, $mediaId, $caption = null)
    {
        ...
        try {
            ...
            if ($jsonData && isset($jsonData['messages']) && count($jsonData['messages']) > 0) {
                $wamid = $jsonData['messages'][0]['id'] ?? null;
                Log::info("[WhatsApp] ✅ Video sent - Message ID: " . ($wamid ?? 'unknown'));
                return ['success' => true, 'message' => 'Video đã gửi thành công', 'whatsapp_message_id' => $wamid];
            }
    ...
    public function sendAudioMessage($toPhone, $mediaId)
    {
        ...
        try {
            ...
            if ($jsonData && isset($jsonData['messages']) && count($jsonData['messages']) > 0) {
                $wamid = $jsonData['messages'][0]['id'] ?? null;
                Log::info("[WhatsApp] ✅ Audio sent - Message ID: " . ($wamid ?? 'unknown'));
                return ['success' => true, 'message' => 'Âm thanh đã gửi thành công', 'whatsapp_message_id' => $wamid];
            }
    ...
    public function sendStickerMessage($toPhone, $mediaId)
    {
        ...
        try {
            ...
            if ($jsonData && isset($jsonData['messages']) && count($jsonData['messages']) > 0) {
                $wamid = $jsonData['messages'][0]['id'] ?? null;
                Log::info("[WhatsApp] ✅ Sticker sent - Message ID: " . ($wamid ?? 'unknown'));
                return ['success' => true, 'message' => 'Sticker đã gửi thành công', 'whatsapp_message_id' => $wamid];
            }
    ...
    /**
     * Gửi Read Receipt (đánh dấu tin nhắn đã đọc) - 2 tick xanh
     * @param string $messageId - WhatsApp Message ID (wamid)
     * @return array
     */
    public function sendReadReceipt(string $messageId): array
    {
        $url = "{$this->apiUrl}/{$this->phoneId}/messages";

        Log::info("[WhatsApp] Sending READ RECEIPT for message ID: {$messageId}");
        ...
        try {
            ...
            if ($response->successful()) {
                Log::info("[WhatsApp] ✅ Read receipt sent successfully for: {$messageId}");
                return [
                    'success' => true,
                ...
                Log::error("[WhatsApp] ❌ Read receipt failed: {$error}");
                return [
                    'success' => false,
                    'message' => $error
                ];
            ...
            Log::error("[WhatsApp] ❌ Read receipt exception: " . $e->getMessage());
            return [
                'success' => false,
                'message' => $e->getMessage()
            ];
